...
export default function TeamBuilderPage() {
  ...
  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
    {teams.map((team) => (
      <Card
        key={team.team_id}
        className="flex w-1/2 flex-row items-center justify-between rounded-lg border p-4"
      >
        <Link
        ...
        <Dialog open={isDialogOpen && editingTeamId === team.team_id} onOpenChange={setIsDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <Button
                ...
                onClick={async () => {
                  if (editingTeamName.trim()) {
                    await updateTeam(editingTeamId!, editingTeamName);
                    const updatedTeams = await getTeams(user!.id);
                    setTeams(updatedTeams);
